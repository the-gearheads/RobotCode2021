...
public class StevenOperate extends OperatorProfile {
    ...
    public void createBinds(XboxController controller, Joystick joystick, StreamDeck streamdeck, Subsystems s) {
        ...
        buttons[1].setMode("hold").whileHeld(shootAt(s, 5000));
        buttons[2].setMode("hold").whileHeld(shootAt(s, 5500));
        buttons[3].setMode("hold").whileHeld(shootAt(s, 6000));
        buttons[4].setMode("hold").whileHeld(shootAt(s, 6500));
    }

    public Command shootAt(Subsystems s, double rpm) {
        return (new Shoot(s.shooter, rpm).withTimeout(1)).andThen(new Elevate(s.elevator).alongWith(new Shoot(s.shooter, rpm)));
    }
